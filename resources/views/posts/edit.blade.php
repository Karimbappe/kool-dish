...
<form method="POST" action="{{ route('posts.update', $post) }}" enctype="multipart/form-data">
    ...
    <form method="POST" action="{{ route('posts.store') }}" enctype="multipart/form-data">
        ...
        <p>
            ...
            <input type="text" name="title" value="{{ isset($post->title) ? $post->title : old('title') }}" id="title"
        ...
        <p>
            ...
            <textarea name="content" id="content" lang="fr" rows="10" cols="50"
                placeholder="Le contenu du post">{{ isset($post->content) ? $post->content : old('content') }}</textarea>
